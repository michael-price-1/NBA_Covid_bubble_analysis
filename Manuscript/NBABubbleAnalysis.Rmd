...
\emph{need to give some descriptive statistics on how many games in
each sample}
...
comparing away scoring in 2020 to 2017-2019. These tests will also be simultaneously
conducted using the Wilcoxon Test. The advantage of running the Wilcoxon Test is as a
...
2020 to that of 2017-2019 and once for each oftwo-point shooting, three-point shooting and
free throws for both home and away teams in 2020 vs. 2017-2019. Again these tests will
run simultaneously as a z-test and a Wilcoxon test.
...
Table \@ref(tab:table) summarizes results of the nine hypotheses
including the testing statistics and their p-values. Also reported are
the point estimates of the two samples in each comparison and the
confidence intervals of the true difference between the two.
...
multiple tests using the Bonferroni correction, the p-value is no longer significant.
...
2017-19. There appear to be no evidence suggesting home teams played at
a lower level in 2020 than they did in previous years when they had home-court advantage.


\emph{focus on the two tests that are significant after Bonferroni
correction.}
