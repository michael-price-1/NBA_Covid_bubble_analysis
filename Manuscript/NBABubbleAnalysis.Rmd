...
which the home teams are accustomed to playing with @Lopez. Examples
...
percentages. @Kotecki reported significant evidence of home-court
...
@Harris used two-point shots, three-point shots and free
...
on scoring was conducted by @Jones, he analyzes scoring patterns across
...
interesting finding cited by @Carron2005 is that the absence of crowds results
in overall performance increases. Another study by @Price is able 
...
Lastly, as discussed in the introduction, @Lopez
...
There are a few examples of natural experiments in basketball. @Harville studied the
...
concluded with evidence supporting home-court advantage. Also, @Boudreaux 
...
crowd and @Boudreaux estimate crowd effects increase the chance
...
# pdf("Fig1.pdf")
p <- ggplot(winyear, aes(x=years, y=winpct))+
    geom_point()+
...
ggsave("Fig1.pdf", plot = p, height = 4.2, width = 6)
```
...
ggsave("Fig2.pdf", plot = p, height=4.2, width = 6)
```
...
ggsave("Fig3.pdf", plot = p, height=4.2, width = 6)
```
...
ggsave("Fig4.pdf", width=6, height=4.2)
```
